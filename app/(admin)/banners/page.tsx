  ...
  return (
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        ...
        <button
          onClick={openNew}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#1A0A00] text-[#C8880A] text-sm font-semibold hover:bg-[#2A1508] transition-colors"
        >
            ...
            <div className="p-6 space-y-4">
              ...
              <Field label="On tap action">
                <select className={input} value={editing.action?.type || 'none'}
                  ...
                  <option value="category">Filter menu category</option>
                </select>
              </Field>
              {editing.action?.type !== 'none' && (
                <Field label={editing.action?.type === 'restaurant' ? 'Restaurant ID' : editing.action?.type === 'url' ? 'URL' : 'Category name'}>
                  <input className={input} value={editing.action?.value || ''}
                    placeholder={editing.action?.type === 'restaurant' ? 'e.g. abc123' : editing.action?.type === 'url' ? 'https://...' : 'e.g. Grills'}
