...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    return () => { unsubRestaurants(); unsubOrders(); unsubUsers() }
  }, [])

  const activeRestaurants = restaurants.filter(r => (r.status ?? 'active') === 'active').length
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatsCard label="Active Restaurants" value={activeRestaurants}
          sub={pendingRestaurants > 0 ? `${pendingRestaurants} pending approval` : undefined}
          icon={Store} />
        ...
        <div className="mb-6 p-4 rounded-xl bg-yellow-50 border border-yellow-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-yellow-800">
              {pendingRestaurants} restaurant application{pendingRestaurants > 1 ? 's' : ''} waiting for approval
            </span>
          </div>
          <a href="/restaurants?filter=pending" className="text-sm font-semibold text-yellow-700 hover:underline">
            Review →
          </a>
        </div>
      )}
              ...
              <div key={order.id} className="px-6 py-4 flex items-center justify-between hover:bg-zinc-50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#1A0A00] flex items-center justify-center flex-shrink-0">
                    <ShoppingBag size={14} className="text-[#C8880A]" />
                  </div>
                  <div>
