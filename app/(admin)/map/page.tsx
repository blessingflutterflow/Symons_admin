...
export default function OperationsMapPage() {
  ...
  const onRestaurantClickRef = useRef<(r: Restaurant) => void>(() => {});
  const onDriverClickRef = useRef<(d: DriverLocation) => void>(() => {});
  onRestaurantClickRef.current = (r) => { setSelectedDriver(null); setSelectedRestaurant(r); };
  onDriverClickRef.current = (d) => { setSelectedRestaurant(null); setSelectedDriver(d); };

  const GOOGLE_MAPS_API_KEY = "AIzaSyB4wHFe2xOgiBKAXmoENZbHwfa-bMQaE-U";
  ...
  // Subscribe to restaurants — state only
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "restaurants"), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        if (status === "active" && typeof data.lat === "number" && typeof data.lng === "number") {
          restaurantData.push({
            id: doc.id,
  ...
  // Draw restaurant markers whenever map becomes ready OR restaurant data changes
  useEffect(() => {
    if (!mapLoaded) return;
  ...
  const updateRestaurantMarkers = (restaurantList: Restaurant[]) => {
    if (!mapInstanceRef.current) return;
    const map = mapInstanceRef.current;
    const existingMarkers = restaurantMarkersRef.current;

    existingMarkers.forEach((marker, id) => {
      if (!restaurantList.find((r) => r.id === id)) {
        marker.setMap(null);
        existingMarkers.delete(id);
      }
    });
    ...
    restaurantList.forEach((restaurant) => {
      ...
      if (existingMarkers.has(restaurant.id)) {
        existingMarkers.get(restaurant.id)!.setPosition(position);
      } else {
        const marker = new google.maps.Marker({
          position, map,
          title: `${restaurant.name} ${restaurant.branch}`.trim(),
          cursor: "pointer",
          icon: {
            url: `data:image/svg+xml,${encodeURIComponent(createRestaurantSvg(restaurant.isOpen))}`,
            scaledSize: new google.maps.Size(42, 42),
            anchor: new google.maps.Point(21, 21),
          },
        });
        marker.addListener("click", () => onRestaurantClickRef.current(restaurant));
        existingMarkers.set(restaurant.id, marker);
      }
    });
  };

  // Driver marker — gold circle with car silhouette
  const createDriverSvg = () => `<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44">
    <circle cx="22" cy="22" r="20" fill="#E0A020" stroke="#1A0A00" stroke-width="2.5"/>
    <!-- car body -->
    <rect x="9" y="24" width="26" height="8" rx="2" fill="#1A0A00"/>
    <!-- car cabin -->
    <path d="M13 24 L16 17 L28 17 L31 24 Z" fill="#1A0A00"/>
    <!-- windscreen -->
    <path d="M17 24 L19 19 L25 19 L27 24 Z" fill="#E0A020"/>
    <!-- rear window -->
    <path d="M14.5 24 L16.5 20 L18 24 Z" fill="#E0A020" opacity="0.5"/>
    <!-- wheels -->
    <circle cx="14" cy="32" r="3.5" fill="#2A1508" stroke="#E0A020" stroke-width="1.5"/>
    <circle cx="30" cy="32" r="3.5" fill="#2A1508" stroke="#E0A020" stroke-width="1.5"/>
    <!-- headlight -->
  ...
  // Restaurant marker — dark pin with store icon
  const createRestaurantSvg = (isOpen: boolean) => {
    const ringColor = isOpen ? "#22c55e" : "#ef4444";
    return `<svg xmlns="http://www.w3.org/2000/svg" width="42" height="42" viewBox="0 0 42 42">
      <circle cx="21" cy="21" r="19" fill="#1A0A00" stroke="${ringColor}" stroke-width="3"/>
      <!-- awning triangle -->
      <path d="M10 19 L21 11 L32 19 Z" fill="#C8880A"/>
      <!-- building body -->
      <rect x="12" y="19" width="18" height="13" fill="#fff" rx="1"/>
      <!-- door -->
      <rect x="17" y="24" width="8" height="8" rx="1" fill="#1A0A00"/>
      <!-- left window -->
      <rect x="13" y="21" width="4" height="3" rx="0.5" fill="#C8880A"/>
      <!-- right window -->
      <rect x="25" y="21" width="4" height="3" rx="0.5" fill="#C8880A"/>
      <!-- open/closed dot -->
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Badge variant="outline" className="bg-green-100 text-green-700 border-green-300">
              ...
              {restaurants.filter((r) => r.isOpen).length} Open Restaurants
            </Badge>
          </div>
        </div>
      </div>
            ...
            <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-lg p-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#E0A020] border border-[#1A0A00]"></span>
