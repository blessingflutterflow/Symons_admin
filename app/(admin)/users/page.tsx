            ...
            <tbody className="divide-y divide-zinc-50">
              {users.map(u => (
                <tr key={u.id} className="hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    {u.role ? <StatusBadge status={u.role} /> : <span className="text-sm text-zinc-400">—</span>}
                  </td>
                  <td className="px-6 py-4 text-sm text-zinc-600 max-w-[220px] truncate">{u.homeAddress || '—'}</td>
